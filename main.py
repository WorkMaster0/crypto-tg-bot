# smc_bot.py — Binance-only multi-symbol, multi-timeframe SMC scanner with pre-top detect
# Requirements: python-binance, pandas, numpy, scikit-learn, joblib, apscheduler, flask, requests
    ...
    for attempt in range(retry):
        try:
            ...
            df = pd.DataFrame(kl, columns=[
                'open_time','open','high','low','close','volume','close_time','qav','num_trades','taker_base_vol',
...
def detect_order_blocks(df, lookback=60):
    ...
    if seg_range < hist_mean*0.6:
        return {'type':'order_block','top':float(last_segment['high'].max()),
                'bot':float(last_segment['low'].min()),'range':float(seg_range)}
    return None
...
def detect_fair_value_gap(df):
    ...
    for i in range(len(df)-3,0,-1):
        ...
        if b_low>a_high and c_low>b_high:
            return {'type':'fvg_up','low':float(a_high),'high':float(b_low)}
        if b_high<a_low and c_high<b_low:
            return {'type':'fvg_down','low':float(b_high),'high':float(a_low)}
    return None
...
def detect_pre_top_pump(df, lookback=6):
    if df is None or len(df)<6: return None
    recent = df.iloc[-lookback:]
    start=recent['close'].iloc[0]; end=recent['close'].iloc[-1]
    pct_move=(end/start)-1
    vol_ratio=recent['volume'].iloc[-1]/(recent['volume'].rolling(lookback).mean().iloc[-1]+1e-9)
    if pct_move>=PUMP_PCT_THRESHOLD and vol_ratio>=2.0:
...
def detect_market_structure_shift(df):
    ...
    prior_high=df['high'].iloc[-10:-2].max(); prior_low=df['low'].iloc[-10:-2].min()
...
def build_training_dataset(df, lookahead=5):
    ...
    df2=df2.dropna(); df2['target']=(df2['future_ret']>0).astype(int)
...
def combine_evidence(evidence, ml_prob):
    ...
    elif mss=='BOS_DOWN': score_short+=0.08
...
def analyze_symbol(symbol, interval='5m'):
    try:
        ...
        return {"symbol":symbol,"interval":interval,"price":price,"evidence":evidence,
                "ml_prob":ml_prob,"ml_acc":ml_acc,"confidence":conf,"direction":direction,
                "label":label,"timestamp":str(df.index[-1])}
    except Exception:
        return None
...
def ema_scan_all(interval='5m'):
    ...
    for r in results_sorted:
        ...
        if ev.get('mss'): patt.append(ev['mss'])
...
def monitor_top_symbols():
    ...
    with ThreadPoolExecutor(max_workers=min(PARALLEL_WORKERS,8)) as exe:
        ...
        for fut in as_completed(futures):
            res=fut.result()
            if not res: continue
            conf=res['confidence']; ml_prob=res['ml_prob']; direction=res['direction']
            should_alert=False
            ...
            if should_alert:
                ...
                if prev!=direction:
                    ev=res['evidence']; patt=[]
                    if ev.get('order_block'): patt.append('order_block')
                    if ev.get('fvg'): patt.append(ev['fvg']['type'])
                    if ev.get('sweep'): patt.append(ev['sweep']['type'])
                    if ev.get('pump'): patt.append('pump')
                    text=f"⚡ *Signal*\nSymbol: `{res['symbol']}`\nInterval: `{res['interval']}`\nSignal: *{direction}* ({res['label']})\nPrice: `{res['price']}`\nConfidence: `{conf:.2f}`\nTime: `{res['timestamp']}`"
                    if ml_prob is not None: text+=f"\n🤖 ML ProbUp: `{ml_prob:.2f}` | ML Acc: `{res['ml_acc'] or 0:.2f}`"
                    if patt: text+=f"\n🧩 Patterns: {', '.join(patt)}"
                    send_telegram_message(text)
...
@app.route('/status')
def status():
    return jsonify({"state":state,"time":str(datetime.now(timezone.utc))})
...
def warmup():
    ...
    for s in syms:
        ...
        if df is not None: train_or_load_model(s,df,lookahead=5)
...
if __name__=="__main__":
    ...
    except Exception as e: logger.exception("Initial scan failed: %s", e)
