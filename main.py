# main.py — Покращений Pre-top бот з графіками та Telegram
...
def fetch_klines(symbol, interval="15m", limit=EMA_SCAN_LIMIT):
    for attempt in range(3):
        try:
            if not client: raise RuntimeError("Binance client unavailable")
...
def detect_signal(df: pd.DataFrame):
    ...
    # Pre-top
    pretop=False
    if len(df)>=10 and (df["close"].iloc[-1]-df["close"].iloc[-10])/df["close"].iloc[-10]>0.1:
...
def plot_signal_candles(df,symbol,action,votes,pretop):
    ...
    df_plot.index.name="Date"; addplots=[]
    last=df.iloc[-1]
    patterns={"bullish_engulfing":"green","bearish_engulfing":"red","hammer_bull":"lime","shooting_star":"orange","doji":"blue","fake_breakout_short":"darkred","fake_breakout_long":"darkgreen"}
    if pretop: addplots.append(mpf.make_addplot([df['close'].iloc[-i] for i in range(3,0,-1)],type='scatter',markersize=80,marker='^',color='magenta'))
    for pat,color in patterns.items():
        if pat in votes: addplots.append(mpf.make_addplot([last['close']]*len(df),type='scatter',markersize=50,marker='o',color=color))
    # Entry/stop/takes
    ...
    take_levels=[]
    if entry and stop:
        if action=="LONG": move=last['resistance']-entry; take_levels=[entry+move*0.25,entry+move*0.5,entry+move*0.75]
        elif action=="SHORT": move=entry-last['support']; take_levels=[entry-move*0.25,entry-move*0.5,entry-move*0.75]
    if entry:
        addplots.append(mpf.make_addplot([entry]*len(df),type='scatter',markersize=30,marker='v',color='blue'))
        addplots.append(mpf.make_addplot([stop]*len(df),type='scatter',markersize=30,marker='x',color='red'))
        for tl in take_levels: addplots.append(mpf.make_addplot([tl]*len(df),type='scatter',markersize=20,marker='^',color='green'))
    ...
    mpf.plot(df_plot,type='candle',style=s,volume=True,addplot=addplots,
             ...
             savefig=dict(fname=buf,dpi=100,bbox_inches='tight',facecolor='white'),tight_layout=True)
...
def analyze_and_alert(symbol:str):
    ...
    # Save win stats
    win_stats=state.get("win_stats",{}); win_stats.setdefault(symbol,{"total":0,"wins":0})
    win_stats[symbol]["total"]+=1
    if action!="WATCH" and confidence>=CONF_THRESHOLD_MEDIUM:
        if pretop or "strong_trend" in votes: win_stats[symbol]["wins"]+=1
    state["win_stats"]=win_stats
    # Save history
    hist=state.get("signal_history",{}); hist.setdefault(symbol,[]); hist[symbol].append({"time":str(datetime.now(timezone.utc)),"action":action,"price":last['close'],"confidence":confidence}); state["signal_history"]=hist
    save_json_safe(STATE_FILE,state)
    if pretop: send_telegram(f"⚡ Pre-top detected for {symbol}, price={last['close']:.6f}")
...
def scan_top_symbols():
    ...
    if not symbols: logger.warning("No symbols found for scanning."); return
    logger.info("Starting scan for %d symbols",len(symbols))
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as exe: list(exe.map(analyze_and_alert,symbols))
...
scheduler.add_job(scan_top_symbols,"interval",minutes=max(1,SCAN_INTERVAL_MINUTES),id="scan_job",next_run_time=datetime.now())
scheduler.start(); logger.info("Scheduler started with interval %d minutes",SCAN_INTERVAL_MINUTES)

# ---------------- FLASK ----------------
@app.route("/")
def home(): return jsonify({"status":"ok","signals":len(state.get("signals",{}))})

@app.route(f"/telegram_webhook/<token>",methods=["POST","GET"])
def telegram_webhook(token):
    if token!=TELEGRAM_TOKEN: return jsonify({"ok":False,"reason":"invalid token"}),403
    if request.method=="POST":
        update=request.get_json(force=True)
        if "message" in update:
            msg=update["message"]; text=msg.get("text","").lower()
            if text.startswith("/scan"): Thread(target=scan_top_symbols,daemon=True).start(); send_telegram("Manual scan started.")
                ...
                else: send_telegram("No win stats available yet.")
    return jsonify({"ok":True})

# ---------------- AUTO WEBHOOK ----------------
def auto_register_webhook():
    if WEBHOOK_URL and TELEGRAM_TOKEN: logger.info("Registering Telegram webhook: %s",WEBHOOK_URL); set_telegram_webhook(WEBHOOK_URL)
Thread(target=auto_register_webhook,daemon=True).start()

# ---------------- WARMUP ----------------
def warmup_and_first_scan():
    try: scan_top_symbols()
    except Exception as e: logger.exception("warmup_and_first_scan error: %s", e)
